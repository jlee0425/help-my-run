...
- Test: `go test ./internal/store/ ./internal/progress/ ./internal/api/ ./internal/coach/ ./internal/chat/ ./internal/metrics/`
...
- [ ] **Step 6: Run — expect PASS for the re-keyed packages.** Command: `cd /home/jake/project/help-my-run/backend && go build ./... 2>&1 | head -30 && go test ./internal/store/ ./internal/progress/ ./internal/api/ ./internal/coach/ ./internal/chat/ ./internal/metrics/ 2>&1 | tail -30`. Expected: `go build ./...` succeeds (all packages compile — `sync`/`agent`/`streams` still reference strava + garmin_activities Go symbols which all still exist, and `mapActivity` now uses `ActivityID`). `internal/store` PASS (incl `TestMigration00009RekeysAndDrops` + new `wantTables`); `internal/progress`/`internal/api`/`internal/coach`/`internal/chat`/`internal/metrics` PASS.
> NOTE: `go test ./...` will still FAIL the TEST build of `internal/sync`/`internal/streams` (whole-file rewrites owned by STRIP Tasks 5/6) — `sync/streams_sync_test.go` + `streams/engine_test.go` still reference `StravaID:`/old `New` signatures. The CORE gate here is `go build ./...` green + the six re-keyed packages' tests green.
...
- [ ] **Step 2: Delete the `strava` package + `oauth_state.go`.** Command:
```
